...
pub fn generate_image(seed: u32, fmt: &PixFmt, width: u32, height: u32) -> Result<MyYCbCrImage> {
    if (fmt == &PixFmt::Mono12) || (fmt == &PixFmt::Rgb12) {
        ...
        let values_mono12: Vec<u16> = (0..width)
            .map(|idx| ((((idx + seed) % width) as f64) * max_value / (width - 1) as f64) as u16)
            .collect();
    ...
    let image_row_mono8: Vec<u8> = (0..width)
        .map(|idx| ((((idx + seed) % width) as f64) * max_value / (width - 1) as f64) as u8)
        .collect();
